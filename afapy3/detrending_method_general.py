...
def detrending_method_general(data1 , data2 , seg_len , fit_order) :

	nrows , ncols = shape(data1)
	if nrows > ncols :
		data1 = data1.T

	nrows , ncols = shape(data2)
	if nrows < ncols :
		data2 = data2.T

	if size(data1) != size(data2) :
		print('Error:The two input data have to be of the same length!')
...
# seg_len = 1001,odd number
	...
	for seg_index in range(1 , 2) :
		...
		xi_left_index = mat(xi)
		xi_max = xi.max()
		xi_min = xi.min()
		xi_left = data1[0 , xi_min - 1 : xi_max]
		seg_data = data2[xi_min - 1 : xi_max , 0]
		xi_left_1 = array(xi_left)
		slope = np.polyfit(array(xi_left).flatten() , array(seg_data).flatten() , fit_order)
		left_trend = mat(np.polyval(slope , xi_left))
		# mid trend
		...
		if seg_index * (seg_len - 1) + 1 + nonoverlap_len > data_len :
			...
			xi_max = xi.max()
			xi_min = xi.min()
			xi_mid = data1[0 , xi_min - 1 : xi_max]
			seg_data = data2[xi_min - 1 : xi_max , 0]
			slope = np.polyfit(array(xi_mid).flatten() , array(seg_data).flatten(), fit_order)
			mid_trend = mat(np.polyval(slope , xi_mid))
			...
			mid_start_index = mat([(j) for j in range(shape(xi_mid_index)[1]) if xi_mid_index[0 , j] == xi_left_index[0 , shape(xi_left_index)[1] - 1] + 1])
			...
			detrended_data = data2 - record_y.T
		...
		#right trend
			...
			if (seg_index + 1) * (seg_len - 1) + 1 > data_len :
				...
				xi_right_index = xi_right
				xi_max = xi.max()
				xi_min = xi.min()
				xi_right = data1[0 , xi_min - 1 : xi_max]
				seg_data = data2[xi_min - 1 : xi_max , 0]
				slope = np.polyfit(array(xi_right).flatten() , array(seg_data).flatten() , fit_order)
				right_trend = mat(np.polyval(slope , xi_right))
				...
				right_start_index = mat([(j) for j in range(shape(xi_right_index)[1]) if xi_right_index[0 , j] == xi_mid_index[0 , shape(xi_mid_index)[1] - 1] + 1])
